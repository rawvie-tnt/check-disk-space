...
async function checkZfsPool(dependencies = {
    ...
    try {
        const { stdout } = await dependencies.cpExecFile("zpool", ["list", "-o", "free", "-H"], { windowsHide: true });
        const pools = stdout
            .split("\n")
            .map((line) => line.trim())
...
export { InvalidPathError, NoMatchError, checkZfsPool, checkDiskSpace as default, getFirstExistingParentPath };
